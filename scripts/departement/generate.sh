...
do
	docker exec -u $UID be4-$DEP bash scripts_be4/SCRIPT_${i}.sh &
done

wait

docker exec -u $UID be4-$DEP bash scripts_be4/SCRIPT_FINISHER.sh

docker exec -u $UID be4-$DEP create-layer.pl --pyr=pyramids/descriptors/BDORTHO-5M-${DEP}.pyr --tmsdir=./ --layerdir=pyramids/descriptors/
